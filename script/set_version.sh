...
if [[ "$OSTYPE" == "linux-gnu"* ]]; then
  sed -i -r "s/DEFAULT_IMAGE :=.*/DEFAULT_IMAGE := ${sanitized_image_name}:${version}/" $location/Makefile
elif [[ "$OSTYPE" == "darwin"* ]]; then
  # Mac OSX
  sed -i '' -E "s/DEFAULT_IMAGE :=.*/DEFAULT_IMAGE := ${sanitized_image_name}:${version}/" $location/Makefile
fi

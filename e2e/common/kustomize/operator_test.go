...
func TestAlternativeImageOperator(t *testing.T) {
	...
	WithNewTestNamespace(t, func(ns string) {
		...
		newTag := "1.1.1"
		ExecMake(t, Make("operator", fmt.Sprintf("IMAGE_NAME=%s", newImage), fmt.Sprintf("TAG=%s", newTag), fmt.Sprintf("NAMESPACE=%s", ns)))
...
func TestGlobalOperator(t *testing.T) {
	...
	WithNewTestNamespace(t, func(ns string) {
		ExecMake(t, Make("setup-cluster", fmt.Sprintf("NAMESPACE=%s", ns), "GLOBAL=true"))
